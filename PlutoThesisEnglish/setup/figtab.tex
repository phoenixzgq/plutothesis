...
\else%
  {\ifoneortwoside\clearpage{\
  pagestyle{empty}\cleardoublepage}\else\newpage\fi} % 清除目录后面空页的页眉和页脚
\fi
\addcontentsline{toc}{chapter}{\textbf{List of Figures} }   % 中文插图加入到中文目录
...
\fi
\addcontentsline{toc}{chapter}{\textbf {List of Tables}}   % 中文表格加入到中文目录
