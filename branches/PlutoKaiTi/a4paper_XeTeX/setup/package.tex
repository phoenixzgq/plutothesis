...
\usepackage[%paperwidth=18.4cm, paperheight= 26cm,  %  版面控制宏包，定义规定的版面尺寸
            body={453.55true pt,700.15true pt},           % 论文版芯145mm×210mm（包括页眉及页码则为145mm×230mm）
            twosideshift=0 pt,                      % 页码在版芯下边线之下隔行居中放置；
            %headheight=1.0true cm
            ]{geometry}                             % body（宽度-左边距-右边距） （高度-上边距-下边距）
\usepackage{layouts}                    % 打印当前页面格式的宏包
\usepackage[sf,bf]{titlesec}               % 控制标题的宏包
...
\usepackage[boxed,linesnumbered]{algorithm2e}    % 算法的宏包
